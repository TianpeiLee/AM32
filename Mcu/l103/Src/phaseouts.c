...
void proportionalBrake()
{
    ...
    PHASE_A_GPIO_PORT_HIGH->CFGHR &= ~(0xf<<0); PHASE_A_GPIO_PORT_HIGH->CFGHR |= (0x3<<0);
    ...
    PHASE_C_GPIO_PORT_HIGH->CFGHR &= ~(0xf<<8); PHASE_C_GPIO_PORT_HIGH->CFGHR |= (0x3<<8);
    PHASE_C_GPIO_PORT_HIGH->HIGH_BITREG_OFF = PHASE_C_GPIO_HIGH;


    // set low channel to PWM, duty cycle will now control braking
    PHASE_A_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<28); PHASE_A_GPIO_PORT_LOW->CFGLR|= (0xb<<28);
    PHASE_B_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<0); PHASE_B_GPIO_PORT_LOW->CFGLR|= (0xb<<0);
    PHASE_C_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<4); PHASE_C_GPIO_PORT_LOW->CFGLR|= (0xb<<4);
}
...
void phaseCPWM()
{
    if (!eepromBuffer.comp_pwm)
    {
        PHASE_C_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<4); PHASE_C_GPIO_PORT_LOW->CFGLR |= (0x3<<4);
    ...
    PHASE_C_GPIO_PORT_HIGH->CFGHR &= ~(0xf<<8); PHASE_C_GPIO_PORT_HIGH->CFGHR |= (0xb<<8);
}
...
void phaseAPWM()
{
    if (!eepromBuffer.comp_pwm)
    {
        PHASE_A_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<28); PHASE_A_GPIO_PORT_LOW->CFGLR|= (0x3<<28);
        PHASE_A_GPIO_PORT_LOW->LOW_BITREG_OFF = PHASE_A_GPIO_LOW; //low close
    }
    else
    {
        PHASE_A_GPIO_PORT_LOW->CFGLR  &= ~(0xf<<28); PHASE_A_GPIO_PORT_LOW->CFGLR|= (0xb<<28);//low pwm
    }
    PHASE_A_GPIO_PORT_HIGH->CFGHR &= ~(0xf<<0); PHASE_A_GPIO_PORT_HIGH->CFGHR |= (0xb<<0); //high pwm
}
