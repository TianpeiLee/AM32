 ...
 *      Author: Alka
 *
 *      Modified By TempersLee Step. 26, 2025.
 */
...
void AT_COMP_Init(void)
{
     ...
     SetVTFIRQ( (uint32_t)EXTI3_IRQHandler,EXTI3_IRQn,1,ENABLE  );  
...
void TIM1_Init(void)
{
    ...
    RCC_PB2PeriphClockCmd( RCC_PB2Periph_TIM1|RCC_PB2Periph_AFIO, ENABLE );
...
void TenkHz_Init(void) //TEN_KHZ_TIMER
{
    // SysTick->CTLR = 0;
    // SysTick->SR   = 0;
    // SysTick->CNT  = 0;
    ...
    LPTIM_TimeBaseInitStruct.LPTIM_InClockSource = LPTIM_InClockSource_HSI;
    ...
    LPTIM_TimeBaseInitStruct.LPTIM_Period = HSI_VALUE / LOOP_FREQUENCY_HZ - 1;

    LPTIM_TimeBaseInit( & LPTIM_TimeBaseInitStruct);
    LPTIM_ITConfig(LPTIM_IT_ARRM, ENABLE);

    NVIC_SetPriority(LPTIM_IRQn, 0xE0);
...
void UTILITY_TIM_Init(void)
{
    UTILITY_TIMER->CTLR = 0;
    UTILITY_TIMER->SR = 0;
    UTILITY_TIMER->CNT = 0; 
    UTILITY_TIMER->CMP = 0xFFFFFFFF;
    UTILITY_TIMER->CTLR = 0b1101;    
}
...
{
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_AFIO | RCC_PB2Periph_GPIOA | RCC_PB2Periph_GPIOB, ENABLE);
}
...
{
    return INTERVAL_TIMER->CNT_TIM4;
}

void setintervaTimerCount(uint16_t intertime)
{
    INTERVAL_TIMER->CNT_TIM4 = intertime;
}
